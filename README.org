mlr3resampling provides new cross-validation algorithms for the mlr3 framework in R ([[https://mlr3resampling.netlify.app/][documentation on netlify]])
...
For an overview of functionality, please read
the [[https://onlinelibrary.wiley.com/doi/10.1002/sam.70055][SOAK paper]],
[[https://tdhock.github.io/blog/2024/cv-all-same-new/][my recent blog post]],
[[https://mlr3resampling.netlify.app/articles][package vignettes]],
and [[https://github.com/tdhock/mlr3resampling/wiki/Articles][other articles]].

*** SOAK: Same/Other/All K-fold cross-validation for estimating similarity of patterns in data subsets

See examples in [[https://mlr3resampling.netlify.app/doc/newer_resamplers][Newer resamplers vignette]] and data viz for
...
See my [[https://docs.google.com/presentation/d/1gXEqaPQBdJxA39iLqy6SlpA1LZGHcOOgJ1dT1_lzVlo/edit?slide=id.g3ef515ab464_0_0#slide=id.g3ef515ab464_0_0][slides]] for empirical comparisons of different packages which implement stratified CV with groups.

| Software | group | strata | subset | group + stratum | group + stratum + subset |
|----------|-------|--------|--------|-----------------|--------------------------|
| mlr3     | yes   | yes    | no     | no              | no                       |
| caret    | yes   | yes    | no     | no              | no                       |
|splitTools| yes   | yes    | no     | no              | no                       |
| origami  | yes   | yes    | no     | random+leakage (only 1 stratum per group)   | no                       |
| rsample  | yes   | yes    | no     | random (only 1 stratum per group)   | no                       |
| bioLeak  | yes   | yes    | no     | random             | no                       |     |
| sklearn  | yes   | yes    | no     | yes             | no                       |
| soakpy   | no    | no     | yes    | no              | no                       |
|mlr3resampling|yes|yes     | yes    | yes             | yes                      |
...
- [[https://scikit-learn.org/stable/modules/generated/sklearn.model_selection.StratifiedGroupKFold.html][StratifiedGroupKFold]] in scikit-learn implements a heuristic for assigning folds that attempts to minimize standard deviation between folds (averaged over classes/strata). This heuristic is implemented in mlr3resampling as ResamplingSameOtherSizes with =group_stratum_algo= set to Wasikowski.
- [[https://github.com/mlr-org/mlr3spatiotempcv][mlr3spatiotempcv]] provides a common interface to several R packages which compute spatial blocks, which could in principle be used as column role=group in SOAK/mlr3resampling, but not in practice because [[https://github.com/mlr-org/mlr3spatiotempcv/issues/251][the block variable is not public]]. Such spatial blocks are typically assigned using a grid or randomly (k-means), and these block ID numbers (1,2,…) could be used as column role=subset in SOAK/mlr3resampling (as long as the blocks are big enough to train the Same model), but it is typically better to assign column role=subset to a more interpretable categorical variable (city, state, province, country, etc.).
- My PHD student Tung Nguyen wrote [[https://github.com/lamtung16/soak_regression?tab=readme-ov-file#usage][python code for SOAK]].
- As of May 2026, [[https://github.com/tdhock/mlr3resampling.viz][mlr3resampling.viz]] contains older resamplers which are only useful for visualization: ResamplingSameOtherCV, ResamplingVariableSizeTrainCV.
