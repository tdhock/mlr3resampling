...
The result above includes test AUC and accuracy values, which are more convincing (but this involves actually submitting jobs on the cluster).
...
For both the previous and proposed approaches, we use the multisession plan for parallel computation using all local CPUs.
...
```{r}
if(requireNamespace("mlr3batchmark") && slurm.available){
...
Even though the torch learner has max 1000 epochs, it is reduced to only 2 epochs when running `proj_test()`.
