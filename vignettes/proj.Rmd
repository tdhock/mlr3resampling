...
```{r}
test_result_list$learners_weights.csv[weight!=0]
```

The table above contains one row for each non-zero weight in the glmnet model (these are the features selected by L1 regularization; the other features are not used for prediction).
