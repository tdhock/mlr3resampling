---
vignette: >
  %\VignetteIndexEntry{5 Newer resamplers}
...
Before we create a task from these data, we need to load the `mlr3resampling` package, or we will get an error like below, because this is not a standard mlr3 role.
...
Below we assign the random group column to
...
```{r simulationShort, fig.height=2}
N <- 600
abs.x <- 20
set.seed(1)
