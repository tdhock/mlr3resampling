...
for(score.i in 1:nrow(reg.bench.score)){
  ...
  set.ids <- data.table(
    set=c("test","train")
  )[, data.table(row_id=reg.bench.row[[set]][[1]]), by=set]
  i.points <- set.ids[task.dt, on="row_id"][is.na(set), set := "unused"]
  point.dt.list[[score.i]] <- data.table(
    reg.bench.row[, .(pattern=task_id, iteration)],
    i.points)
  i.learner <- reg.bench.row$learner[[1]]
  pred.dt.list[[score.i]] <- data.table(
    reg.bench.row[, .(
      pattern=task_id, iteration, algorithm
    )],
    as.data.table(
...
viz <- animint(
  ...
  pred=ggplot()+
    ...
    geom_line(aes(
      x, y, color=algorithm, group=algorithm),
      showSelected="iteration",
      data=pred.dt)+
    facet_grid(
      pattern ~ person,
  ...
  err=ggplot()+
    ...
    facet_grid(
      ...
      labeller=label_both,
      scales="free"),
  source="https://github.com/tdhock/mlr3resampling/blob/8fb4174152bbf4ea2dfa5fbce8a4983c155baac9/vignettes/Simulations.Rmd")
if(FALSE){
